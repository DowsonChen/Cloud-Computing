...
-- start query1_opt
select sum(lo_extendedprice*lo_discount) as revenue from lineorder_opt 
JOIN (select * from dwdate_opt where d_year = 1997) as dwdate_opt 
where lo_orderdate=d_datekey 
and lo_discount between 1 and 3 
and lo_quantity < 24;
-- end query1_opt
...
-- start query2_opt
select sum(lo_revenue), d_year, p_brand1 from lineorder_opt
JOIN (select * from part_opt where p_category = 'MFGR#12') as part_opt 
JOIN (select * from supplier_opt where s_region = 'AMERICA') as supplier_opt 
JOIN dwdate_opt
where lo_orderdate = d_datekey
and lo_partkey = p_partkey 
and lo_suppkey = s_suppkey
...
-- start query3_opt
select c_city, s_city, d_year, sum(lo_revenue) as revenue from lineorder_opt
JOIN (select * from dwdate_opt where d_yearmonth = 'Dec1997') as dwdate_opt 
JOIN (select * from customer_opt where (c_city='UNITED KI1' or c_city='UNITED KI5')) as customer_opt 
JOIN (select * from supplier_opt where (s_city='UNITED KI1' or s_city='UNITED KI5')) as supplier_opt 
where lo_custkey = c_custkey 
and lo_suppkey = s_suppkey and lo_orderdate = d_datekey 
group by c_city, s_city, d_year order by d_year asc, revenue desc limit 5;
-- end query3_opt
...
credentials 'aws_access_key_id=;aws_secret_access_key=' gzip compupdate off;
